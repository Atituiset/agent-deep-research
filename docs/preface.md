...
> **书名与术语**：本书名为 **Agent Deep Research**（研究项目名），研究对象是 **Agent Infra**（Agent 基础设施）。书中统一用 **Harness** 指代"模型之外、Agent 边界之内"的运行与治理层——它与 Agent Infra 同义，只是更强调工程构件视角。读到这两个词时请当作一回事。
